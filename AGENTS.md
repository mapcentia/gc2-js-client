...
- `createCentiaAdminClient(config)` (`src/admin.ts`) wraps a `CentiaHttpClient` with one resource class per provisioning area (`src/provisioning/`: Schemas, Tables, Columns, Constraints, Indices, Sequences, Users, Clients, Rules, Layers, Maps, Privileges, RpcMethods, Functions, MetadataWrite, TypeScriptInterfaces, FileImport, GitCommit). Each class takes the http client in its constructor and maps to `/api/v4/...` endpoints.
- OGC service wrappers (`src/ogc/`: `Ows`, `Wfs`, `Mapcache`) follow the same explicit-client pattern — instantiated with a `CentiaHttpClient` — but are exported standalone from the barrel rather than via the admin client, since they are runtime data services, not provisioning.
